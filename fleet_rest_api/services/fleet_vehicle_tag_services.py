...
class FleetVehicleTagService(Component):
    ...
    _name = "fleet.vehicle.tag.service"
    _usage = "fleet_vehicle_tag"
    _expose_model = "fleet.vehicle.tag"
    _description = """
    Fleet Vehicle Tag Services
    """

    @restapi.method(
        routes=[(["/<int:id>"], "GET")],
        output_param=Datamodel("fleet.vehicle.tag.output"),
    )
    def get(self, _id):
    ...
    @restapi.method(
        routes=[(["/search"], "GET")],
        input_param=Datamodel("fleet.vehicle.tag.search.input"),
        output_param=Datamodel("fleet.vehicle.tag.search.output"),
    )
    def search(self, filters):
        domain = self._get_base_search_domain(filters)
        records = self.env[self._expose_model].search(domain)
        result = {"size": len(records), "data": self._to_json(records, many=True)}
        return self.env.datamodels["fleet.vehicle.tag.search.output"].load(result)

    @restapi.method(
        routes=[(["/create"], "POST")],
        input_param=restapi.Datamodel("fleet.vehicle.tag.input"),
        output_param=restapi.Datamodel("fleet.vehicle.tag.output"),
    )
    ...
    @restapi.method(
        routes=[(["/update"], "POST")],
        input_param=Datamodel("fleet.vehicle.tag.input"),
    )
