...
class VehicleTagService(Component):
    ...
    def search(self, description):
        ...
        """
        vehicles = self.env["fleet.vehicle.tag"].name_search(description)
        vehicles = self.env["fleet.vehicle.tag"].browse([i[0] for i in vehicles])
        rows = []
    ...
    def create(self, **params):
        ...
        """
        vehicle = self.env["fleet.vehicle.tag"].create(self._prepare_params(params))
    ...
    def _get(self, _id):
        return self.env["fleet.vehicle.tag"].browse(_id)
